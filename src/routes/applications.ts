...
router.get('/companies/:id/reputation', async (req, res) => {
    try {
        ...
        const overallScore = (
            reputation.responseTimeScore * 0.3 +
 ...
 */
function calculateRepputationBadges(reputation: any): string[] {
    const badges = [];

    if (reputation.responseTimeScore > 90) badges.push('Quick Responder');
    if (reputation.ghostingRate < 5) badges.push('No Ghosting');
    if (reputation.candidateSatisfaction > 4.5) badges.push('Candidate Favorite');
    if (reputation.totalApplications > 100) badges.push('High Volume Employer');
    if (reputation.averageResponseTime && reputation.averageResponseTime < 24) badges.push('Same Day Response');

...
export class AIMatchingService {
     ...
     */
    async matchDeveloperToJob(developerProfile, jobPosting) {
        try {
     ...
     */
    async getJobRecommendations(developerProfile, availableJobs) {
     ...
     */
    async analyzeWeb3Fit(githubStats, skills) {
        const web3Keywords = [
        ...
        const web3Skills = skills.filter(skill => web3Keywords.some(keyword => skill.name.toLowerCase().includes(keyword)));
        score += web3Skills.length * 15;
        // Check GitHub repos for Web3 projects
        if (githubStats?.topRepos) {
            const web3Repos = githubStats.topRepos.filter((repo) => web3Keywords.some(keyword => repo.name.toLowerCase().includes(keyword) ||
        ...
        if (githubStats?.languages) {
            const relevantLanguages = githubStats.languages.filter((lang) => web3Languages.includes(lang.toLowerCase()));
     ...
     */
    buildMatchingPrompt(developerProfile, jobPosting) {
        return `
DEVELOPER PROFILE:
- GitHub Username: ${developerProfile.githubUsername || 'N/A'}
- Experience: ${developerProfile.yearsOfExperience || 'N/A'} years
- Skills: ${developerProfile.skills?.map((s) => `${s.skill.name} (${s.proficiency}/10)`).join(', ') || 'N/A'}
...
- Required Skills: ${jobPosting.skills?.map((s) => s.skill.name).join(', ') || 'N/A'}
...
`;
     ...
     */
    fallbackMatching(developerProfile, jobPosting) {
        let score = 0;
        const skillMatches = [];
        // Basic skill matching
        if (jobPosting.skills && developerProfile.skills) {
            const jobSkills = jobPosting.skills.map((s) => s.skill.name.toLowerCase());
            const devSkills = developerProfile.skills.map((s) => s.skill.name.toLowerCase());
            const matchingSkills = jobSkills.filter((skill) => devSkills.includes(skill));
...
//# sourceMappingURL=aiMatchingService.js.map
...
export class AntiGhostingService {
     ...
     */
    async setInitialDeadline(applicationId) {
     ...
     */
    async updateDeadline(applicationId, newStatus) {
        let hoursToAdd = 0;
        switch (newStatus) {
            case 'REVIEWED':
     ...
     */
    async processOverdueApplications() {
        const now = new Date();
     ...
     */
    async autoRejectApplication(application) {
        await this.prisma.application.update({
     ...
     */
    async scheduleWarningNotification(applicationId, deadline) {
     ...
     */
    async updateCompanyReputationScore(companyId) {
        ...
        const respondedApplications = applications.filter((app) => app.status !== 'PENDING' && app.status !== 'EXPIRED').length;
        const expiredApplications = applications.filter((app) => app.status === 'EXPIRED').length;
        ...
        const responseTimes = applications
            .filter((app) => app.status !== 'PENDING' && app.lastStatusUpdate)
            .map((app) => {
     ...
     */
    async requestCandidateFeedback(applicationId) {
        const application = await this.prisma.application.findUnique({
            where: { id: applicationId },
            include: {
                job: { include: { company: true } },
        ...
        if (!application || application.feedbackGiven)
            return;
        await this.createNotification({
            applicationId,
            recipientId: application.userId,
     ...
     */
    async generateCommunicationSuggestion(applicationId, context) {
        try {
            const application = await this.prisma.application.findUnique({
                where: { id: applicationId },
                include: {
    ...
    buildCommunicationPrompt(application, context) {
     ...
     */
    async getApplicationDashboard(applicationId) {
        const application = await this.prisma.application.findUnique({
            where: { id: applicationId },
            include: {
    ...
    calculateEstimatedResponseTime(reputationScore) {
        if (!reputationScore?.averageResponseTime)
...
//# sourceMappingURL=antiGhostingService.js.map